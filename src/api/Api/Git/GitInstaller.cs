...
namespace Unity.VersionControl.Git
{
	...
	public class GitInstaller : TaskBase<GitInstaller.GitInstallationState>
	{
		...
		public class GitInstallationState
		{
            ...
            private GitInstallDetails Defaults { get; }

			public GitInstallationState(IGitEnvironment currentEnvironment)
			{
                Defaults = new GitInstallDetails(currentEnvironment.UserCachePath, currentEnvironment);
            ...
            public void Resolve(GitInstallationState state)
            {
            ...
            private bool ResolvePaths(string execName, NPath pathToExecutable, out NPath execPath, out NPath installPath)
            {
                ...
                if (pathToExecutable.DirectoryExists())
                    pathToExecutable = pathToExecutable.Combine(execName + UnityEnvironment.ExecutableExtension);
                else
                    pathToExecutable = pathToExecutable.Parent.Combine(execName + UnityEnvironment.ExecutableExtension);
		...
		public class GitInstallDetails
		{
         ...
         private const string packageFeed = "http://localhost:50000/unity/git/git.json";
			...
			public GitInstallDetails(NPath baseDataPath, IGitEnvironment environment)
			{
